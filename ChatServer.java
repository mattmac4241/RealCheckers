...
public class ChatServer implements Runnable{
	...
	public synchronized void handle(int ID,String input){
		System.out.println("MESSAGE: " + input + " SIZE:  " + input.length());
		if(input.length() == 0){
		...
		if(input.equals(".bye")){
			...
			if(isConnected(ID) == true){
				if(input.equals("new game")){
					clients[findClient(ID)].send("play 1");
					clients[findClient(u.getConnected())].send("play 2");
							}
				else if(input.substring(0, 6).equals("MOVE: ") & input.length() > 9){
					clients[findClient(u.getConnected())].send(input);
				}
	...
	public void addConnections(int id,int cId){
		if(findClient(cId) == -1){
			...
			user[cCount] = new User();
			user[cCount].setID(id);
			user[cCount].conenctTo(cId);
			cCount+=1;
			user[cCount] = new User();
			user[cCount].setID(cId);
